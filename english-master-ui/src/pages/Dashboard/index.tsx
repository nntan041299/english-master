...
function StatCard({ label, value, icon, accent, sub }: StatCardProps) {
  return (
    <div className="bg-white rounded-2xl p-5 border border-surface-200 flex flex-col gap-3">
      ...
      <div>
        <p className="text-2xl font-bold text-surface-900" style={{ fontFamily: 'var(--font-display)' }}>
          {value}
        </p>
        <p className="text-sm font-medium text-surface-500 mt-0.5" style={{ fontFamily: 'var(--font-sans)' }}>
          {label}
        </p>
        {sub && (
          <p className="text-xs text-surface-400 mt-1" style={{ fontFamily: 'var(--font-sans)' }}>
            {sub}
          </p>
        )}
      </div>
...
function ActivityRow({ label, done }: ActivityRowProps) {
  return (
    <div className="flex items-center gap-3 py-2.5 border-b border-surface-100 last:border-0">
      ...
      <span className="text-sm text-surface-600" style={{ fontFamily: 'var(--font-sans)' }}>
        {label}
      </span>
    </div>
  );
...
const STATS: StatCardProps[] = [
  { label: 'Words learned',  value: '—', icon: 'pi-book',      accent: 'bg-sage-100 text-sage-700'      },
  { label: 'Mastered',       value: '—', icon: 'pi-star',      accent: 'bg-gold-100 text-gold-600'      },
  { label: 'Need review',    value: '—', icon: 'pi-sync',      accent: 'bg-amber-50 text-amber-600'     },
  { label: 'Day streak',     value: '—', icon: 'pi-bolt',      accent: 'bg-ink-900/8 text-ink-900'      },
];
      ...
      <div className="max-w-5xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="bg-ink-900 rounded-2xl px-7 py-6 flex items-center justify-between overflow-hidden relative">
          ...
          <div className="absolute right-0 top-0 w-64 h-full opacity-20"
            style={{ background: 'radial-gradient(ellipse at right, var(--color-gold-500), transparent 70%)' }} />

          <div className="relative">
            <p className="text-sm font-medium text-white/50 mb-1" style={{ fontFamily: 'var(--font-sans)' }}>
              {greeting}
            </p>
            <h1
            ...
            <p className="text-sm text-white/50 mt-1" style={{ fontFamily: 'var(--font-sans)' }}>
              Keep building your vocabulary — every word counts.
            </p>
          </div>

          <div className="relative flex-shrink-0 text-center px-4">
            <p className="text-3xl font-bold text-gold-400" style={{ fontFamily: 'var(--font-display)' }}>0</p>
            <p className="text-xs font-medium text-white/40 uppercase tracking-widest mt-0.5" style={{ fontFamily: 'var(--font-sans)' }}>
              Day streak
            </p>
          </div>
        </div>
