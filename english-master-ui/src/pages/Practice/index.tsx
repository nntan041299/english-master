...
const OPTION_KEYS: OptionKey[] = ['OPTION_1', 'OPTION_2', 'OPTION_3', 'OPTION_4'];
...
function CircleProgress({ current, total }: { current: number; total: number }) {
  ...
  return (
    <div className="relative flex items-center justify-center w-14 h-14">
      ...
      >
        {current}<span className="text-surface-400">/{total}</span>
      </span>
    </div>
  );
}
    ...
    >
      <span className="flex items-center gap-3">
        <span
          ...
          `}
        >
          {optionKey === 'OPTION_1' ? 'A' : optionKey === 'OPTION_2' ? 'B' : optionKey === 'OPTION_3' ? 'C' : 'D'}
        </span>
        {text}
      </span>
    </button>
...
export default function Practice() {
  ...
  const { data: items = [], isLoading: loading, isError: error, refetch } = usePractices();
  ...
  const handleSelect = (key: OptionKey) => {
    ...
    answerMutation.mutate(
      { wordId: current.wordId, practiceId: current.practiceId, selectedOption: key },
      {
        onSuccess: (result) => {
      ...
      <Layout>
        <div className="h-full flex items-center justify-center">
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-sm text-surface-400" style={{ fontFamily: 'var(--font-sans)' }}>Loading practice…</p>
          </div>
        </div>
      </Layout>
    );
      ...
      <Layout>
        <div className="h-full flex items-center justify-center">
          <div className="flex flex-col items-center gap-4 text-center">
            ...
            <p className="text-sm text-surface-600" style={{ fontFamily: 'var(--font-sans)' }}>
      ...
      <Layout>
        <div className="h-full flex items-center justify-center">
          <div className="flex flex-col items-center gap-4 text-center max-w-sm px-6">
            ...
            <h2 className="text-xl font-bold text-surface-900" style={{ fontFamily: 'var(--font-display)' }}>
              All caught up
            </h2>
            <p className="text-sm text-surface-500" style={{ fontFamily: 'var(--font-sans)' }}>
              No words to practice right now. Add more words to your vocabulary and come back.
            </p>
            <button
        ...
        <div className="h-full flex items-center justify-center px-4">
          ...
          >
            ...
            <div className="w-full bg-surface-100 rounded-xl p-4 flex items-center justify-between">
              <span className="text-sm text-surface-500" style={{ fontFamily: 'var(--font-sans)' }}>Accuracy</span>
              <span
      ...
      <div className="h-full flex flex-col items-center justify-center px-4 py-6">
        <div className="w-full max-w-lg flex flex-col gap-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-xs font-semibold text-surface-500 uppercase tracking-widest" style={{ fontFamily: 'var(--font-sans)' }}>
                  Practice
                </span>
                <span className="text-xs text-surface-400" style={{ fontFamily: 'var(--font-sans)' }}>
                  {index + 1} of {total}
                </span>
              </div>
          ...
          >
            ...
            <p className="text-sm text-surface-400 mt-1" style={{ fontFamily: 'var(--font-sans)' }}>
